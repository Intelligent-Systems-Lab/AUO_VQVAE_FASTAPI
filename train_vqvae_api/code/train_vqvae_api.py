...
async def post_train(response: Response, 
    ...
    with lock:
        ...
        config['num_epochs'] = epochs if epochs is not None else config['num_epochs']
...
async def get_train_result(response: Response, background_tasks: BackgroundTasks, job_id: int):
    ...
    if j_idx is None:
        ...
        zip_path = os.path.abspath(os.path.join(job_dir, "{}.zip".format(str(job_id))))
    ...
    if os.path.isdir(j_dir_path):
        with zipfile.ZipFile(zip_path, 'w', zipfile.ZIP_DEFLATED) as zfile:
            ...
            #ip_path = os.path.abspath(os.path.join(job_dir, "{}.zip".format(str(job_id))))
            for f in files:
                zfile.write(os.path.join(j_dir_path, f), f)
        logger.debug("Generated zip_path: %s", zip_path)
